            ...
            <div ref={this.randomItem}
                ...
                {Array.range(1, this.rows).map((i) =>
                    Array.range(1, this.cols).map((j) =>
                        <div
                            key={Grid.toCellIndex(i, j)} className={`just-grid-cell just-random-chess-grid-cell-${Grid.toCellIndex(i, j)} just-grid-cell-${Grid.toCellIndex(i, j)}`}>
                        </div>
                    )
                )}
            </div>
